...
class Head(nn.Module):
    ...
    def forward(self, x, feat=False):
        ...
        if feat:
            return [x0, x1, x2, x3]
        if self.scale != 1.0:
            x3 = F.interpolate(x3, scale_factor=1. / self.scale, mode="bilinear")
        return x3
...
class IFNet(nn.Module):
    ...
    def forward(self, img0, img1, timestep, f0, f1):
        ...
        for i in range(5):
            ...
            upscale = self.scale_list[i] / self.scale_list[i + 1] if i < 4 else self.scale_list[i]
            scale_next = 1 / self.scale_list[i + 1] if i < 4 else 1
